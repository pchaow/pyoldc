import { Tabs, Tab, Input, DatePicker, Skeleton, Textarea, Image, Button, Select, SelectItem } from "@nextui-org/react";
...
function SupportEdit() {
    ...
    const clearImage = async () => {
        setIsUploading(true)
        call.post("pyoldc.pyoldc.doctype.supports.supports.clear_image", {
            'doc': createForm,
        }).then(() => {
    ...
    const uploadFile = async (e: any) => {
        ...
        file.uploadFile(
            ...
            .then((response) => {
                ...
                call.post("pyoldc.pyoldc.doctype.supports.supports.upload_image", {
                            ...
                            <div className="grid grid-cols-2 gap-4 pt-4">
                                <div>
                                    ...
                                    <Skeleton isLoaded={!loading} className="rounded-lg w-full">
                                        <Input variant="bordered" radius="sm" type="date" placeholder="วัน/เดือน/ปีเกิด" name="support_date"
                                            isInvalid={!!(error?.support_date)} errorMessage={error?.support_date}
                                            value={createForm.support_date} onChange={(e) => { updateForm('birthdate', e.target.value) }} />
                                    </Skeleton>
                                </div>
                                <div>
                                    <p>ปีงบประมาณ (พ.ศ.)</p>
                                ...
                                <div>
                                    ...
                                    <Input variant="bordered" radius="sm" type="text" placeholder="กรอกชื่อผู้พิการ" readOnly
                                        name="support_receiver_name" value={createForm.support_receiver_name} />
                                </div>
                                <div>
                                    <p>นามสกุลผู้พิการ</p>
                                    <Input variant="bordered" radius="sm" type="text" placeholder="กรอกนามสกุลผู้พิการ" readOnly
                                        name="support_receiver_surname" value={createForm.support_receiver_surname} />
                                </div>
                                <div className="col-span-2">
                                    ...
                                    <Skeleton isLoaded={!loading} className="rounded-lg w-full">
                                        <Textarea variant="bordered" radius="sm" type="textarea" placeholder="กรอกรายละเอียดความช่วยเหลือ" name="support_detail"
                                            isInvalid={!!(error?.support_detail)} errorMessage={error?.support_detail}
                    ...
                    <div className="-mt-3">
                        <div className="border-solid border-2 rounded-lg shadow p-8 mb-8 mt 9 bg-white">
                            ...
                            <div className="relative z-0 mt-0.5 flex w-full -space-x-px">
                                ...
                                {createForm.support_image ? <Button isLoading={isUploading} onClick={clearImage} className="inline-flex w-auto items-center justify-center rounded-r rounded-l border border-gray-200 bg-white px-3 py-2  text-gray-800 hover:border-gray-300 hover:bg-gray-100 "><RiDeleteBin2Fill className="text-red-500" fontSize="1.3rem" /></Button>
